...
export default function SuperAdminAdmins() {
  const { allUsers, adminUsers, persistUsers, getStatusLabel, formatDateTime } = useSuperAdminData()
  ...
  const onCreate = (e) => {
    ...
    const nextUsers = [
      ...allUsers,
      { name: addForm.name, email: addForm.email, password: addForm.password, role: 'school_admin', active: true },
    ]
    persistUsers(nextUsers)
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {showAddForm && (
          <form onSubmit={onCreate} className="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-4">
            <input value={addForm.name} onChange={(e) => setAddForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="Nama admin" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" required />
            <input value={addForm.email} onChange={(e) => setAddForm((prev) => ({ ...prev, email: e.target.value }))} placeholder="Email admin" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" required />
